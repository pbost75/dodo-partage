...
import { CardRadioOption } from '@/components/ui/CardRadioGroup';
...
const ChoiceModal: React.FC<ChoiceModalProps> = ({ isOpen, onClose, onChoice }) => {
  ...
  const handleDirectChoice = (choice: string) => {
    ...
    // Redirection immédiate vers la page de transition
    onChoice(choiceType);
  };
      ...
      >
        ...
        >
          ...
          <div className="relative px-8 py-6 border-b border-gray-100">
            <button
              onClick={onClose}
              className="absolute right-6 top-6 p-2 transition-colors rounded-full hover:bg-gray-100 text-gray-400 hover:text-gray-600"
            >
              <FaTimes size={16} />
            </button>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {choiceOptions.map((option) => (
                <motion.div 
                  ...
                  className="relative rounded-xl border-2 p-6 transition-all duration-300 ease-out cursor-pointer hover:shadow-md hover:border-gray-300 hover:-translate-y-1 border-gray-200"
