...
import { getPostalCodeSuggestions, CitySuggestion } from '@/utils/cityAutocomplete';
...
interface PostalCodeAutocompleteProps {
  ...
  onPostalCodeSelect?: (suggestion: CitySuggestion) => void;
  country?: string;
  placeholder?: string;
  error?: string;
  required?: boolean;
...
const PostalCodeAutocomplete: React.FC<PostalCodeAutocompleteProps> = ({
  ...
  const triggerSearch = async (searchValue: string) => {
    ...
    if (searchValue.length >= 2 && !disabled && hasFocused) {
      timeoutRef.current = setTimeout(async () => {
        setIsLoading(true);
        try {
          const results = await getPostalCodeSuggestions(searchValue, country);
          ...
          if (inputRef.current && results.length > 0) {
            ...
            const dropdownHeight = Math.min(results.length * 60, 240);
  ...
  const handleSuggestionSelect = (suggestion: CitySuggestion) => {
    ...
    // Mettre à jour la valeur de l'input avec le code postal
    ...
    // Notifier le parent avec la suggestion complète
    if (onPostalCodeSelect) {
      onPostalCodeSelect(suggestion);
    }
