      ...
      `}>
        {options.map((option) => (
          ...
          >
            ...
            {option.emoji && (
              <div className={`mb-4 flex h-12 w-12 items-center justify-center rounded-full flex-shrink-0 
                ${option.iconBgColor || 'bg-blue-50'}`}>
                <span className={`${option.iconTextColor || 'text-blue-500'} text-2xl`}>{option.emoji}</span>
              </div>
            )}
